...
const Line = ({ tokens, row, left }) => {
	...
	return isSelectedLine(selection, row) ? row === selection.start.row && row === selection.stop.row ? (
		<SingleSelectionRow
			tokens={tokens}
			index={index}
		...
		<Anchor tokens={tokens} index={selection.start.index} left={left} focused={row === focusedRow} />
	) : row === selection.stop.row ? (
		<Extent tokens={tokens} index={selection.stop.index} left={left} focused={row === focusedRow} />
	) : (
		<Full tokens={tokens} />
	) : (
		<WithOutSelection focused={row === focusedRow} tokens={tokens} index={index} left={left} />
	);
};
...
const Anchor = ({ tokens, index, left, focused }) => {
	if (tokens.length > 0) {
		...
		const width = tokens.reduce((a, t) => a + t.dom.el.getBoundingClientRect().width, 0);
		return [
			<span
				key="start"
				className="unselected"
			...
			<span className="selected" style={{ width: width - pos + left }} key="selected" />
		];
	}
	return null;
};
...
const isSelectedLine = (selection, row) =>
	selection && selection.start && selection.stop
		? row >= selection.start.row &&
			row <= selection.stop.row &&
			(selection.start.row !== selection.stop.row || selection.start.index !== selection.stop.index)
		: false;

const getToken = (index) => (tokens) => {
	const token = tokens.find(({ start, stop }) => index >= start && index <= stop);

	return token ? token : tokens.length > 0 ? tokens[tokens.length - 1] : undefined;
};
