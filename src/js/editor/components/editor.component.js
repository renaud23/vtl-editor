...
const Editor = ({ parse }) => {
	...
	const visiblesLines = lines.reduce(
		(a, line, i) => (i >= scrollRange.start && i <= scrollRange.stop ? [ ...a, line ] : a),
		[]
	);
	...
	useEffect(
		() => {
			if (editorEl.current) {
				dispatch(actions.setScrollrange(computeScrollRange(editorEl.current, rowHeight)));
			}
		},
	...
	return (
		<div className="editor-container">
			<div ref={editorEl} className="editor">
				{visiblesLines.map(({ tokens, value }, i) => <Line key={`${i}-${value}`} tokens={tokens} row={i} />)}
			</div>
